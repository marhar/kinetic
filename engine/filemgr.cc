...
///////////////////////////////////////ncic
            // since we read the file ourselves, it is just a
            // sequence of bytes.  we must convert that sequence
            // of bytes into utf format before we evaluate it.

            Tcl_DString tmpbody;
            Tcl_DStringInit(&tmpbody);
            Tcl_ExternalToUtfDString(NULL, pf->parsed_body(),
                                 strlen(pf->parsed_body()), &tmpbody);
            //evstat = Tcl_Eval(interp, pf->parsed_body());
fprintf(stderr,"tmpbody=:%s:\n", tmpbody);
            evstat = Tcl_Eval(interp, Tcl_DStringValue(&tmpbody));
            Tcl_DStringFree(&tmpbody);
///////////////////////////////////////ncic
