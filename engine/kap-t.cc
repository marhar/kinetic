...
////////////////////////////////////////////////////////////////
// setUtfVar2 -- convert a string to utf and setvar2 it
////////////////////////////////////////////////////////////////

void setUtfVar2(Tcl_Interp* interp, char* name1, char* name2,
                  char* newValue, int flags)
{
//fprintf(stderr, "setUtfVar2(%s,%s,%s)\n", name1,name2,newValue);
    Tcl_DString qname;
    Tcl_DStringInit(&qname);
    Tcl_ExternalToUtfDString(NULL, newValue, strlen(newValue), &qname);
    Tcl_SetVar2(interp, name1, name2, Tcl_DStringValue(&qname), 0);
    Tcl_DStringFree(&qname);
}
                ...
                int objc, Tcl_Obj *CONST objv[])
{
    ...
    //******** BUG, SHOULD ONLY DO THIS IF STDOUT
    if (clen > 0) {
        headerflush();
    }
...
//NCIC...
    Tcl_Obj *path;
    path = Tcl_NewStringObj("/usr/lib/tcl8.3", -1);
    TclSetLibraryPath(Tcl_NewListObj(1,&path));
    path = TclGetLibraryPath();
...
//... NCIC
    ...
    char srccmd[1000];
    int srcrc;
fprintf(stderr, "checking for init file: %s\n", initfile);
...
void Kaptrans::parsecookies(Tcl_Interp* myinterp, char* str)
{
    ...
    while (*p) {
        ...
        q1 = strchr(q, ';');
        *p1 = *q1 = 0;
        setUtfVar2(interp, "COOKIE", p, q, 0);
        p = q1 + 1;
...
////////////////////////////////////////////////////////////////
// unescapecgi2 -- unescape a cgi sequence
////////////////////////////////////////////////////////////////

char* unescapecgi2(char* out, char* in)
{
    char* pi;
    char* po;
    pi = in;
...
fprintf(stderr, "interp->result=:%s:\n", interp->result);
        ...
        unescapecgi2(q2, q);
        setUtfVar2(interp, "QUERY", p, q2, 0);

        //Tcl_DString q3;
        //Tcl_DStringInit(&q3);
        //Tcl_ExternalToUtfDString(NULL, q2, strlen(q2), &q3);
        //Tcl_SetVar2(interp, "QUERY", p, Tcl_DStringValue(&q3), 0);
        //Tcl_DStringFree(&q3);

    }
fprintf(stderr, "after q2=:%s:\n", q2);
//.................ncic
        p = q1 + 1;
    }
}
...
fprintf(stderr, "query=:%s:\n", val); fflush(stderr);
        ...
        if (strcasecmp(var, "cookie") == 0 && strlen(val) > 0) {
            ...
            if (subkey == 0)
                subkey = var;
            setUtfVar2(interp, "CGI", subkey, val, 0);
        }
    }
